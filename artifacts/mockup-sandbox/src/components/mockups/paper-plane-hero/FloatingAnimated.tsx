export function FloatingAnimated() {
  return (
    <div
    ...
    >
      <style>{`
        ...
        @keyframes plane-float {
          ...
          25%  { transform: translateY(-14px) rotate(-19deg); }
          50%  { transform: translateY(-8px) rotate(-24deg); }
          75%  { transform: translateY(-18px) rotate(-20deg); }
        ...
        .trail-anim {
          animation: trail-fade 7s ease-in-out infinite;
        }
      `}</style>
      ...
      {/* ── Paper Airplane — Flutuando com animação suave ── */}
      <div style={{
        position: "absolute",
        top: "55px",
        right: "60px",
        ...
        {/* Subtle ghost trail to suggest motion history */}
        <div className="trail-anim" style={{ position: "absolute", top: 30, right: -10, opacity: 0.04 }}>
          <svg width="180" height="140" viewBox="0 0 180 140" fill="none">
            <path d="M5 70 L175 5 L120 70 L175 135 Z" fill="#1A3FAB" />
          </svg>
        </div>
        <div className="trail-anim" style={{ position: "absolute", top: 15, right: 5, opacity: 0.05 }}>
          <svg width="200" height="160" viewBox="0 0 200 160" fill="none">
            <path d="M5 80 L195 5 L135 80 L195 155 Z" fill="#1A3FAB" />
          </svg>
        </div>

        {/* Main animated airplane */}
        <div className="paper-plane-anim">
          <svg width="240" height="190" viewBox="0 0 240 190" fill="none" opacity="0.09">
            {/* Main body */}
            <path d="M8 95 L232 8 L160 95 L232 182 Z" fill="#1A3FAB" />
            {/* Belly fold */}
            <path d="M8 95 L160 95 L132 145 Z" fill="#1A3FAB" />
            {/* Wing crease */}
            <line x1="8" y1="95" x2="232" y2="8" stroke="white" strokeWidth="1.5" opacity="0.35" />
            {/* Tip highlight */}
            <circle cx="232" cy="8" r="3" fill="#1A3FAB" opacity="0.5" />
          </svg>
        </div>
      </div>
      ...
      <section style={{ position: "relative", padding: "80px 32px", overflow: "hidden" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 16, overflow: "hidden", background: "rgba(255,255,255,0.7)", marginBottom: 40 }}>
              {[{ v: "7", l: "fases" }, { v: "45+", l: "artefatos" }, { v: "100%", l: "estruturado" }].map((s, i) => (
                <div key={i} style={{ padding: "20px 24px", textAlign: "center", borderRight: i < 2 ? "1px solid rgba(0,0,0,0.07)" : "none" }}>
                  <div className="stat-shimmer-mock3" style={{ fontSize: 28, fontWeight: 700, fontFamily: "'Space Grotesk', sans-serif" }}>{s.v}</div>
        ...
        <div style={{ position: "absolute", bottom: 20, left: "50%", transform: "translateX(-50%)", background: "rgba(26,63,171,0.08)", borderRadius: 999, padding: "6px 16px", fontSize: 11, fontFamily: "monospace", color: "#1A3FAB", letterSpacing: "0.1em", whiteSpace: "nowrap" }}>
          C — FLUTUANDO · ANIMAÇÃO 7s · GHOSTS DE RASTRO · 9% OPACIDADE
        </div>
      </section>
    </div>
  );
